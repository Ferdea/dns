...
def _parse_question(data: BytesIO) -> Question:
    q_name: list[str] = [part.decode() for part in _parse_name(data)]
...
def _parse_answer(data: BytesIO) -> Answer:
    a_name: list[str] = [part.decode() for part in _parse_name(data)]
    ...
    rdata: bytes | list[bytes] = _parse_name(data) \
...
def parse_message(message: bytes) -> Message:
    ...
    questions: list[Question] = \
        [_parse_question(data) for _ in range(header.qd_count)]
    answers: list[Answer] = \
        [_parse_answer(data) for _ in range(header.an_count)]
    authority: list[Answer] = \
        [_parse_answer(data) for _ in range(header.ns_count)]
    additional: list[Answer] = \
        [_parse_answer(data) for _ in range(header.ar_count)]
